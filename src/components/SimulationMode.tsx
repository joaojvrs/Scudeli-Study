...
const SimulationMode = ({ onExit }: { onExit: () => void }) => {
  ...
  const startExam = async () => {
    ...
    try {
      let queryStr = supabase.from('questions').select('*').eq('user_id', supabaseUser.id);
      if (selectedSubject !== 'all') {
        queryStr = queryStr.eq('subject_id', selectedSubject);
      }
      
      const { data: allQs, error } = await queryStr;
      ...
      handleSupabaseError(err, OperationType.LIST, 'questions');
    } finally {
      setIsSubmitting(false);
    }
  };
