...
const MaterialsCenter = () => {
  const { firebaseUser, subjects, tags: globalTags } = useAppContext();
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'materials'), where('userId', '==', firebaseUser.uid));
  ...
  const handleSimulateUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files?.[0] || !firebaseUser) return;
  ...
  const handleProcessAI = async (material: Material) => {
    ...
    try {
      ...
      const questions = await geminiService.generateQuestions(
        subjects.find(s => s.id === material.subjectId)?.name || 'Medicina',
        material.title,
        'medium',
        3
      );
      ...
      await updateDoc(doc(db, 'materials', material.id), { summary });
                ...
                <div className="space-y-2">
                   ...
                   <div className="flex items-center space-x-2 text-xs text-gray-400">
                     ...
                     <span className="bg-gray-100 px-2 py-0.5 rounded-full">
                       {subjects.find(s => s.id === material.subjectId)?.name || 'Geral'}
                     </span>
                   </div>
                </div>
