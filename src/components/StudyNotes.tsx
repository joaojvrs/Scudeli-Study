...
const StudyNotes = () => {
  const { notes, subjects, supabaseUser, tags: globalTags } = useAppContext();
  ...
  const handleCreateNote = async () => {
    ...
    try {
      ...
      handleSupabaseError(err, OperationType.CREATE, 'notes');
    }
  };
        ...
        <header className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
             <select 
               ...
               onChange={(e) => handleUpdateNote(selectedNote.id, { subject_id: e.target.value })}
